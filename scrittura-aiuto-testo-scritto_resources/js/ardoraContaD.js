...
altValue=altValue+"€";
var newCoin= $('<img alt="'+altValue+'" class="coin" src="scrittura-aiuto-testo-scritto_resources/media/'+m[i].substr(1)+'.gif"/>');newCoins.push(newCoin);totPaint=totPaint+values[i];avMoney[i]--;}}
for (i=0; i<newCoins.length;i++){$("#drop").append(newCoins[i]);} paintScore();
var newHtml="";if ($.trim(im[ram_G[indexGame]])){newHtml='<img id="imaL" class="imaLeft" alt="'+imalt[ram_G[indexGame]]+'" src="scrittura-aiuto-testo-scritto_resources/media/'+im[ram_G[indexGame]]+'">';}
newHtml=newHtml+te[ram_G[indexGame]];$("#actText").html(newHtml);
$("#imaL").attr("tabindex","0");if ($.trim(im[ram_G[indexGame]])) {$( "#imaL" ).css("cursor","pointer");$( "#imaL" ).click(function() {imaUp("scrittura-aiuto-testo-scritto_resources/media/G" + im[ram_G[indexGame]],imalt[ram_G[indexGame]])});
$("#imaL").keydown(function(e){if (e.which!=9){ imaUp("scrittura-aiuto-testo-scritto_resources/media/G" + im[ram_G[indexGame]],imalt[ram_G[indexGame]])}});
}
$("#actText").css("minHeight",imaH[ram_G[indexGame]]);$(".buttonAu").remove();
if ($.trim(a1[ram_G[indexGame]])){$("#auDiv").append('<img alt="Play" class="buttonAu" src="scrittura-aiuto-testo-scritto_resources/media/sound.png"/>');
...
if ((totCentFixed == totCent) && (totEurosFixed == totEuros)){isCorrect = true}
